...
const ICONS = {
    ...
    // Contact
    'phone': '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6.62,10.79C8.06,13.62,10.38,15.94,13.21,17.38l2.2-2.2c0.27-0.27,0.67-0.36,1.02-0.24c1.12,0.37,2.33,0.57,3.57,0.57 c0.55,0,1,0.45,1,1v3.5c0,0.55-0.45,1-1,1C10.07,21.5,2.5,13.93,2.5,4c0-0.55,0.45-1,1-1H7c0.55,0,1,0.45,1,1 c0,1.24,0.2,2.45,0.57,3.57C7.98,10.12,7.89,10.52,7.62,10.79L6.62,10.79z"/></svg>',
...
class InstitutDiagram {
    ...
    renderGroups() {
        ...
        const ags = this.data.gruppen.filter(g => g.type === 'ag' && !g.status);

        // TODO: exclude professors without current courses
        ...
        const agsWithCount = ags.map(gruppe => {
            const professors = this.data.personen.filter(p =>
                ...
                (!p.status || p.status.toLowerCase() == 'aktiv' || p.status.toLowerCase() == 'emeritiert-aktiv')
            );
    ...
    showGroupDetail(groupId) {
        ...
        // WiMis (bottom)
        if (wimis.length > 0) {
            html += '<div class="wimis"><h4>Wissenschaftliche Mitarbeiter</h4><div style="display: flex; flex-wrap: wrap;">';
            wimis.forEach(wimi => {
